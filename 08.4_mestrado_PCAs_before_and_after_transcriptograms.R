...
# Função corrigida para extrair variância e variância cumulativa corretamente
get_variance_df <- function(pca_object, label) {
  if (!("explained_variance" %in% names(pca_object)) | !("cumulative_variance" %in% names(pca_object))) {
    warning(paste("Objeto PCA inválido para", label, "- ignorado."))
    return(NULL)
  }
  
  n <- length(pca_object$explained_variance)  # Número de PCs
  
  data.frame(
    PC = seq_len(n),
    Variance = pca_object$explained_variance,
    CumulativeVariance = pca_object$cumulative_variance,
    Analysis = label
  )
}

# Criar os dataframes para cada PCA corretamente
var_df_before_t <- get_variance_df(pca_result_R0, "Before_T")  
var_df_r0 <- get_variance_df(pca_result_R0, "After_R0")
var_df_r30 <- get_variance_df(pca_result_R30, "After_R30")

# Remover dataframes nulos
variance_data <- bind_rows(var_df_before_t, var_df_r0, var_df_r30) %>% na.omit()

# Paleta de cores
color_palette <- c(
  "Before_T" = "#984EA3",  # Roxo
  "After_R0" = "#E41A1C",  # Vermelho
  "After_R30" = "#377EB8"  # Azul
)

# Função para gerar e salvar gráficos com os valores de porcentagem nas barras
plot_variance <- function(data, pc_limit, filename) {
  data_filtered <- filter(data, PC <= pc_limit)
  
  p <- ggplot(data_filtered, aes(x = PC, fill = Analysis)) +
    geom_bar(aes(y = Variance * 100), stat = "identity", alpha = 0.8, position = position_dodge(width = 0.9)) +
    geom_text(aes(y = Variance * 100, label = sprintf("%.1f%%", Variance * 100)), 
              position = position_dodge(width = 0.8), 
              vjust = -0.5, size = 2.5) +  # Adiciona os valores das barras
    geom_line(aes(y = CumulativeVariance * 100, color = Analysis, group = Analysis), size = 1) +
    geom_point(aes(y = CumulativeVariance * 100, color = Analysis), size = 2) +
    labs(title = paste("Explained and Cumulative Variance by Top", pc_limit, "PCs"),
         x = "Principal Component", 
         y = "Variance Explained (%)") +
    scale_x_continuous(breaks = seq(1, pc_limit, by = ifelse(pc_limit <= 10, 1, 10))) +
  ...
  # Salvar gráfico
  ggsave(filename = filename, plot = p, dpi = 300, width = 12, height = 6)
}

# Gerar e salvar os gráficos com os valores de variância explicada
plot_variance(variance_data, 10, "images/explained_cumulative_variance_top10.png")
plot_variance(variance_data, 100, "images/explained_cumulative_variance_top100.png")
plot_variance(variance_data, max(variance_data$PC), "images/explained_cumulative_variance_all.png")
